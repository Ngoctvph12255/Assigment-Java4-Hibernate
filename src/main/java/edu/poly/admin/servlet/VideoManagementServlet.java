...
public class VideoManagementServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		video.setPoster("/PolyAsg/images/3.jpg");

		findAll(request, response);
		request.setAttribute("video", video);

		PageInfo.prepareAndForward(request, response, PageType.VIDEO_MANAGEMEMT_PAGE);
	...
	private void edit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (id == null) {
			request.setAttribute("error", "Video is inserted");
			PageInfo.prepareAndForward(request, response, PageType.VIDEO_MANAGEMEMT_PAGE);
			return;
		}
		try {
