...
public class FavoriteDao extends AbstractEntityDao<Favorite> {
		...
		public List<FavoriteUserReport> reportFavoritesByVideo(String videoId){
		...
		public List<FavoriteReport> reportsByVideo(){
			String jpql = "select new edu.poly.domain.FavoriteReport(f.video.title, count(f) "+
		" , min(f.likeDate), max(f.likeDate)) from Favorite f group by f.video.title";
			EntityManager em = JpaUtils.getEntityManager();
